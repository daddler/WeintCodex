...
- **Item primary stats look exactly like enchants.** The client abbreviates item *secondary* stats ("Kritischer Trefferwert"), so those fall out of the parser by themselves — but "Ausdauer", "Beweglichkeit", "Stärke", "Intelligenz" read identically on an item and in an enchant, and the item's line sits *above* the enchant's in the tooltip. Taking the first plausible green line therefore reported "+1300 Stärke" as the enchant on a strength item. Candidate lines are ranked instead (`RankEnchantCandidate`: name hit → stat hit → key overlap → plausible magnitude), and among equal ranks the last line wins only at the weakest rank, because that is where the enchant sits.
