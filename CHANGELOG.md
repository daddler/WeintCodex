...
**Neu: die Einkaufsliste steht dort, wo man einkauft.** Was fehlt, wusste das Addon längst — nur auf einer Seite, die man vor dem Auktionshaus aufmacht und dann zur Hälfte vergisst.

### Neu
- **Ab Gegenstandsstufe 520 fragt WeintCodex einmal je Charakter, ob es dort von selbst mitreden soll.** Nein heißt nicht „Addon aus": `/wc` öffnet es weiterhin vollständig, und alles, was du selbst aufrufst, funktioniert unverändert. Nein heißt: hier fängt es kein Gespräch mehr an. Was das im Einzelnen abschaltet, steht in der Frage selbst — eine Frage, deren Antwort man nur durch Ausprobieren erfährt, ist keine. Umkehrbar unter *Einstellungen → Fenster & Ansicht*, die Schwelle ebenfalls; `/wc hier` stellt die Frage erneut
- **Einkaufsliste am Auktionshaus.** Sie öffnet sich mit dem Auktionshaus und zeigt, welche Steine für leere oder falsch besetzte Sockel und welche Verzauberungen dir fehlen. **Ein Klick auf eine Zeile sucht danach** — das ist der eigentliche Grund, warum die Liste hier besser aufgehoben ist als auf einem Zettel. Gleiche Steine werden gezählt und in einer Zeile zusammengefasst. Auch über `/wc einkauf`, abschaltbar in den Einstellungen
- **Was auf die Liste kommt, ist ein Einkauf und kein Befund.** Ein leerer Sockel, ein Stein, der nachweislich falsch sitzt, eine fehlende Verzauberung. Ein Stein mit dem Urteil „ok" ist eine Abwägung und steht auf der Sockelseite — eine Einkaufsliste, auf der Dinge stehen, die man nicht braucht, wird nicht benutzt

### Behoben
- **Die eigene Priorisierung wirkte beim Umschmieden nicht.** Der Plan hält sein Ergebnis fest, bis sich an Ausrüstung, Spezialisierung oder Kampfwertung etwas ändert — und eine geänderte Gewichtung ändert nichts davon. Er hat die neuen Gewichte deshalb nie zu Gesicht bekommen und den alten Plan weitergereicht. Jetzt zählen die Gewichte und die selbst gesetzten Tempo-Ziele mit, und beide Seiten werfen den Plan beim Speichern weg
- **Windläufer-Mönch: Meisterschaft war mit 55 deutlich unter Krit (68) gewichtet** — während die eigene gelbe Steinliste den Meisterschafts-Stein an erster Stelle führt. Das ist genau der Widerspruch, den die Selbstprüfung beim Login meldet: die kuratierte Liste entscheidet, das Gewicht sagte etwas anderes. Meisterschaft steht jetzt bei 66
- **Beim Umschmieden steht an jeder Grenze, wieviel noch fehlt oder darüber liegt** — und was den Rest schließt. „Erreicht" allein verschweigt einen Überschuss von 300, und genau der ist die Wertung, die verschenkt wird. Umschmieden bewegt je Gegenstand 40 % *eines* Sekundärwerts, also einen festen Brocken; eine Grenze exakt zu treffen ist damit nicht immer möglich. Was übrig bleibt, schließt ein Stein — deshalb steht die Zahl da

### Technisch
- **`core/optin.lua`** (lädt nach `core/onboarding.lua`, braucht dessen Vorgänger nicht, aber `core/ui.lua` und `core/names.lua`). `OI.Active()` ist die eine Frage, die die vier proaktiven Flächen stellen — Ausrüstungs-Alarm, Rotationshelfer, Umschmieder-Fenster, Einkaufsliste. **Im Zweifel ja:** ohne Antwort, ohne Namen, ohne alles verhält sich das Addon wie vorher; dieselbe Zurückhaltung wie bei `Can()` in `core/access.lua`. Der Charakterschlüssel kommt aus `core/names.lua`, damit „ich" hier dasselbe heisst wie überall sonst. Gefragt wird nach `PLAYER_ENTERING_WORLD` plus Vorlauf und **einmal je Sitzung** — bei `PLAYER_LOGIN` meldet der Client die Gegenstandsstufe oft noch als 0, und eine 0 ist eine Aussage über den Ladezustand, keine über den Charakter
- **`modules/shoppinglist.lua`** rechnet nichts: es liest `WeintCodex.Charakter.Scan()` und zählt zusammen. Eine zweite Bewertung nebenher wäre die Doppelung, an der die Sockelbewertung über fünf Releases gescheitert ist. Die Suche geht über `BrowseName` + `AuctionFrameBrowse_Search` — der Klick ist das Hardware-Ereignis, also ist das erlaubt; gefragt wird trotzdem erst, ob es die Felder gibt (die Oberfläche des Auktionshauses wird nachgeladen), und **findet der Client sie nicht, sagt die Zeile das**, statt stillschweigend nichts zu tun
- **`Signature()` in `modules/reforge_engine.lua` trägt jetzt die wirksamen Gewichte und die selbst gesetzten Schwellenziele.** Was den Suchlauf steuert, muss in seiner Kennung stehen — sonst ist der Zwischenspeicher eine Antwort auf eine andere Frage. Zusätzlich werfen `SetStatTarget` und die Priorisierungs-Seite den Plan ausdrücklich weg, damit die Änderung sofort und nicht erst beim nächsten Ausrüstungswechsel sichtbar wird
- **Der Suchlauf selbst wurde nicht angefasst**, und das ist eine Aussage: gegen rohe Gewalt gemessen trifft er in jedem geprüften Fall exakt dasselbe Ergebnis — auch beim Überschuss über einer Grenze. Der Rest, der am Kap übrig bleibt, ist nicht Ungenauigkeit, sondern die Körnung des Umschmiedens. Der Testlauf unter `.github/tests/` prüft das jetzt auch für die Priorisierung: eine geänderte Gewichtung muss den Plan ohne erzwungenen Lauf ändern
...
**Und *Alles umschmieden* läuft runder.** Wo die angelegte Umschmiedung am Gegenstand steht, fragt das Addon jetzt zuerst das Spiel selbst — statt danach zu suchen.

### Neu
- **Verzauberungen, Sockel und Umschmieden erklären sich.** Rechts neben der Liste steht auf allen drei Seiten dasselbe Feld: deine Spezialisierung, die vier am höchsten gewichteten Werte mit ihren Zahlen, jede Grenze und Schwelle mit dem, was bis dahin noch fehlt oder schon darüber liegt — und was auf dieser Seite überhaupt entschieden wird
- **Die Reihenfolge steht dabei.** Erst umschmieden, dann sockeln, dann verzaubern: Umschmieden kostet nur Gold und lässt sich zurücknehmen, ein Sockel ist einmal vergeben. Dazu, welche Grenze das Umschmieden schon von selbst schließt — dafür braucht es dann keinen Stein mehr. Ohne diesen Satz sieht ein Krit-Stein auf einem Charakter weit unter dem Trefferkap nach Willkür aus
- **Jede Zeile beim Umschmieden trägt ihren Grund ausgeschrieben**, samt der Zahl, um die es geht: „füllt Trefferwertung (es fehlen 138)" statt „füllt Trefferwertung". Der Grund stand bisher rechtsbündig in eine Zeile gequetscht und war damit auf ein Schlagwort beschränkt — und ein Schlagwort beantwortet die Frage nicht, wegen der man hinsieht
- **Auch „nichts tun" hat jetzt einen Grund**, und es sind drei verschiedene: von dir gesperrt, kein umschmiedbarer Wert auf dem Teil, oder Verschieben brächte hier schlicht nichts
- **Eine Verzauberung mit dem Befund *nicht ideal* sagt, was daran fehlt.** Das war der häufigste Befund der Seite und der einzige ohne Auskunft
- **Die Spalte rechts rollt.** Sie war eine feste Fläche: was nicht hineinpasste, lief unten aus dem Fenster heraus — unsichtbar und unerreichbar, und ausgerechnet dann, wenn dort am meisten steht. Die Leiste erscheint nur, wenn es wirklich etwas zu rollen gibt

### Behoben
- **Wo die angelegte Umschmiedung im Gegenstand steht, fragt das Addon jetzt zuerst das Spiel.** Es hat die Stelle bis hierher gesucht und im Zweifel geraten; findet es sie nicht, hält es jeden Gegenstand für nicht umgeschmiedet — der Plan sieht trotzdem vernünftig aus, und nur die Bestätigung nach einem Auftrag kommt nie an. Das Spiel zerlegt Gegenstände selbst und gibt die Stelle heraus; gesucht wird nur noch, wenn es schweigt
- **Der Umschmiede-Bildschirm hakt nicht mehr.** Er hat viermal je Sekunde alle Zeilen neu aufgebaut, auch wenn sich nichts getan hatte, und dabei jedes Mal jeden Gegenstand neu befragt. Jetzt wird nur gezeichnet, wenn wirklich ein Teil fertig geworden ist
- **Wer während des Laufs etwas anlegt, bekommt das Teil übersprungen** statt einer Umschmiedung, die für einen anderen Gegenstand gedacht war. Das ist der eine Fehler, der Gold kostet und erst am Ergebnis auffällt — er wird nicht abgewogen
- **Von Hand umschmieden aktualisiert die Liste.** Sie blieb stehen, bis man etwas anderes anfasste
- **Waffenkunde zählt für Zauberer jetzt auch im Istwert als Zaubertreffer.** Sie lief in die Rechnung nur als *Änderung* ein, nicht als das, was schon dasteht — wer Waffenkunde wegschmiedete, verlor damit Zaubertreffer, den die Ausgangslage nie gutgeschrieben hatte

### Technisch
- **Der Detailbereich zeichnet in ein Bildlauffeld** (`InspectorBody` in `core/navigation.lua`), dessen Inhaltshöhe am Ende von `SetInspector` aus dem gelaufenen `y` steht. Derselbe Fehler und dieselbe Abhilfe wie beim Changelog-Popup in `core/onboarding.lua`. Die 10 px der Leiste gehen von `INSPECTOR_CONTENT_W` ab, und zwar als Zahl statt als Messung: die Blöcke werden gebaut, bevor der Rahmen seine endgültige Größe meldet — dieselbe Reihenfolgenfalle wie beim Detailbereich selbst
- **`WeintCodex.Charakter.Rationale(page)`** in `modules/charakter.lua` baut die Begründungsebene für alle drei Seiten aus **einer** Quelle. Sie rechnet nichts: sie liest `CapContext()` und `RE.CapOutlook()` und formuliert, was dort schon steht. Drei Fassungen davon wären drei Gelegenheiten auseinanderzulaufen — dieselbe Regel, aus der es `PlanItem` und `Raids.ShouldInvite()` gibt. Der Detailbereich hat dafür einen neuen Blocktyp `text`, dessen Höhe **gemessen** wird (`InspectorText` in `core/navigation.lua`); `InspectorCard` rechnet mit festen 15 px je Zeile und kann keinen umbrechenden Satz tragen
- **Der Umschmiedewert im Item-Link kommt aus `GetItemInfoFromHyperlink` + `LinkUtil.SplitLinkOptions` (Feld 10)** — genau der Weg, den ReforgeLite seit Jahren geht, und der Grund, warum es diese Fehlerklasse dort nicht gibt. Die gelernte Position und die Feldsuche aus 2.7.0.2 bleiben als Rückfallweg darunter, in dieser Reihenfolge; die Diagnose sagt je Zeile, welcher gegriffen hat („vom Client zerlegt" gegen „geraten")
- **Jede Antwort wird am Link gemerkt** (`RE.ForgetLinks` wirft den Speicher weg). Der Link trägt den Umschmiedewert selbst, der Speicher ist damit von selbst richtig — und er ist nötig, nicht bequem: an dieser Frage hing im schlechtesten Fall ein Tooltip-Scan je Zeile und Takt. Der Tooltip wird jetzt höchstens einmal je Link gefragt
- **`RunState()` in `modules/reforge.lua`** ist der Stand, an dem sich das Fenster misst — ein Zeichen je geplanter Zeile. Neu gezeichnet wird nur bei Abweichung: WoW gibt Frames nie wieder frei, und ein Neuaufbau je Weckruf war eine Sammlung, die nur wächst
- **Der Slot wird vor dem Auftrag gegen die Planzeile geprüft** (Item-ID). Die laufende Nummer hängt an den Werten des Grundgegenstands und stammt deshalb weiterhin aus dem Plan — diese Ableitung hält aber nur, solange in dem Slot noch derselbe Gegenstand steckt
- **Im Istwert wird die Waffenkunde-Umwandlung mitgerechnet**, wie in ReforgeLites eigenem Getter. Das Ziel ist „Istwert plus Abstand" und verschiebt sich um denselben Betrag mit — der Abstand aus `modules/charakter.lua` bleibt also unverändert; es ändert sich nur, dass Ausgangslage und Änderungen jetzt dasselbe meinen. Willenskraft steht bewusst nicht dabei: wo sie als Zaubertreffer zählt, weist der Charakterbogen sie bereits aus
- **Kopflose Prüfung unter `.github/tests/`** (`lua5.1 .github/tests/reforge_engine_test.lua .`). Sie stellt den Suchlauf gegen eine **rohe Gewalt über alle Kombinationen** — inklusive des Falls, in dem sich am Kap erst die zweite Änderung lohnt und eine gierige Suche davor stehenbleibt — und prüft die drei Stellen, an denen ein Fehler Gold kostet: die laufende Nummer für alle 56 Paare gegen die Regel des Clients nachgerechnet, das Lesen des Umschmiedewerts auf beiden Wegen, und die Waffenkunde-Umwandlung auf beiden Seiten der Rechnung. Der Ordner liegt unter `.github/`, weil die Release-Workflows genau den aus dem Addon-ZIP heraushalten
...
Das Werkzeug ist eine **Beta** und deshalb **ab Werk aus**. Es gibt Gold aus, und die Datenlage dahinter (Spec-Gewichte, Cap-Prozentsätze) ist am laufenden Client nicht geprüft.

### Neu
- **Umschmieden, Sockel und Verzauberungen rechnen ab jetzt miteinander.** Das ist die eigentliche Wirkung dieser Fassung, und sie behebt die häufigste Sorte falscher Sockelempfehlung: **Umschmieden kostet Gold, ein Sockel ist einmalig.** Bis hierher rechnete die Sockelseite mit dem Abstand zum Trefferkap, den sie *gerade* sah — und empfahl deshalb Treffersteine für eine Lücke, die das Umschmieden umsonst schließt. Jetzt ist der Spielraum „was fehlt **nach** dem Umschmieden", und der Sockel geht an den Wert, der wirklich noch etwas bringt. Dieselbe Rechnung trägt die Verzauberungsempfehlung
- **Und in die andere Richtung:** ein Stein über dem Cap gilt nicht mehr als verschwendet, wenn der Umschmiede-Plan den Überschuss ohnehin verschiebt. Er steht dann nicht falsch *im Stein*, er ist der Ausgangspunkt jener Rechnung
- **Jede Empfehlung sagt jetzt, warum.** Unter jeder Zeile auf *Sockel* und *Verzauberungen* steht der Grund — abgelesen an der Entscheidung, die tatsächlich gefallen ist: welcher Wert die Wertung anführt, welcher höher gewichtete dabei ausgeschieden ist und weshalb (*Kap erreicht*, *hinter der Tempo-Schwelle*, *das erledigt das Umschmieden*), und wie die Sockelbonus-Entscheidung ausgegangen ist. Auch das Urteil über einen angelegten Stein: „Bringt 72 % der Wertung des empfohlenen Steins" ist ein Befund, „Tempo zählt hier nicht mehr — die Wertung liegt brach" ist eine Auskunft
- **Die Seite *Charakter → Umschmieden*.** Oben, was die Grenzen deiner Spec sagen — Istwert, Stand nach dem Plan und das Ziel auf einer Bahn. Darunter je Teil, was von wo nach wo geht, wieviel das ist, und **warum**: *füllt Trefferwertung*, *über Waffenkunde*, *Waffenkunde zählt als Trefferwertung (Zauber)*, oder welches Gewicht das andere schlägt. Die Begründung wird nicht dazugeschrieben, sie steht an der Entscheidung, die getroffen wurde
- **Ein Klick auf eine Zeile nimmt das Teil aus der Planung** — für Ausrüstung, die aus einem Grund so bleiben soll, den diese Rechnung nicht kennt. Gesperrte Teile fasst auch der Umschmieder nicht an
- **Beim Umschmieder geht ein Fenster mit derselben Liste auf**, und *Alles umschmieden* erledigt sie in einem Zug. Jedes Teil wird einzeln abgegeben, sobald der Umschmieder das vorige bestätigt hat — der Server gibt den Takt vor. Bleibt eine Antwort aus, bricht der Lauf nach vier Sekunden mit einer Meldung ab, statt still stehenzubleiben
- **Der Reiter *Einstellungen → Umschmieden*** mit dem Hauptschalter, dem Schalter für das Fenster beim Umschmieder, der Zahl der gesperrten Teile und dem Weg zurück
- **`/wc umschmieden`** öffnet die Seite, `an`/`aus` schaltet, `fenster` zeigt die Liste auch ohne Umschmieder, und **`/wc umschmieden prüfen`** druckt jede Zwischenzahl: gelesene Itemwerte, Aufwertungsstufe samt ihrer Quelle, Umwandlungen, das gerechnete Ziel je Wert und die laufende Nummer, die der Umschmieder bekäme

### Geändert
- Die Unterseite steht nur da, wenn der Planer eingeschaltet ist. Ein ausgegrauter Reiter verspricht etwas, das nicht da ist
- Die Sockelseite nennt die Reihenfolge, die aus der neuen Rechnung folgt — **erst umschmieden, dann sockeln** — und verlinkt in beide Richtungen. Ohne diesen Satz sieht „Krit" auf einem Charakter weit unter dem Trefferkap nach Willkür aus
- *Werteverteilung & Caps* sagt bei einem überschrittenen Cap, ob der Umschmiede-Plan ihn schon wegräumt, statt den Befund kommentarlos verschwinden zu lassen
- **`/wc sockel`** druckt den Umschmiede-Ausblick mit aus: Ist- und Zielwert je Grenze und der Spielraum davor und danach. Ohne ihn rechnete die Diagnose mit anderen Zahlen als die Seite

### Technisch
- **`WeintCodex.Charakter.CapContext()`** liefert Spec-Profil, Caps und Tempo-Treppe ohne den vollen Ausrüstungs-Scan. Das spart dem Planer sechzehn Tooltips je Lauf — vor allem aber löst es den Kreis auf: die Sockelplanung liest das Ergebnis des Planers, und riefe der Planer den vollen Scan, riefe der Scan wieder ihn
- **`RE.CapOutlook()`** ist die eine Auskunft, die die Sockelplanung braucht: je Grenze der Stand nach dem Plan. Drei Zurückhaltungen, und jede ist eine Aussage über unser Wissen — nur ein **fertiger** Plan zählt (während gerechnet wird, gibt es keine Auskunft), nur ein Plan zur **aktuellen** Ausrüstung (Kennungsvergleich), und es wird dabei **nie** gerechnet. Ist der Planer aus, kommt `nil` und alles bleibt, wie es vor 2.7.0.0 war
- **Es bleibt bei einem Spielraum.** `headroom` in `ScanCharacter` ist weiterhin *die* Antwort auf „wieviel bringt dieser Wert noch" — jetzt nur richtig gemessen. Ein zweiter Topf neben dem ersten wäre die Doppelung, an der die Sockelbewertung über fünf Releases gescheitert ist
- **Die Begründung entsteht in `PlanItem`, nicht in der Zeichenfunktion.** Ein Text, der in der Anzeige noch einmal hergeleitet wird, ist eine zweite Rechnung — und die läuft irgendwann auseinander. `ExplainGem` liest die Verzweigungen ab, die gefallen sind; `BlockedReason` trennt die drei Gründe für „zählt nicht mehr", weil ein Text für alle drei bei zweien falsch wäre
- **Der Verschwendungs-Topf ist der Rest nach dem Plan.** Bis hierher der Abstand zum Kap, wie er gerade dasteht; damit galt ein Stein als verschwendet, dessen Wertung das Umschmieden gerade verschiebt
- **Der Maßstab ist ReforgeLite, und das ist eine Vorgabe.** Ein zweites Werkzeug, das „ungefähr auch" umschmiedet, ist kein Fortschritt, sondern ein zweiter Ratgeber, dem man nicht trauen kann. Drei Dinge folgen daraus: derselbe Suchlauf, exakte Itemwerte, vollständige Statumwandlungen — jeweils unten
- **Der Suchlauf ist eine vollständige dynamische Programmierung**, keine gierige Näherung. Alles ausser den Grenzwerten ist an jedem Gegenstand linear und fällt in eine einzige Punktzahl zusammen; nur an einer Grenze ist es das nicht, weil Wertung über dem Ziel nichts mehr bringt. Genau diese (höchstens zwei) Achsen bilden den Zustandsraum. Eine gierige Suche findet hier nachweislich nicht das Optimum: am Kap lohnt sich häufig erst die zweite Änderung, und wer Slot für Slot das jeweils Beste nimmt, bleibt davor stehen. Weil der Zustandsraum gerastert ist, rechnet danach eine zweite Stufe mit den **exakten** Summen weiter (erst jeder Slot einzeln, dann je zwei zusammen) und räumt die Rasterreste weg
- **Zwei Achsen reichen:** in `data/spec_profiles.lua` hat keine Spec mehr als zwei Grenzen, und keine der sieben Specs mit Tempo-Treppe hat mehr als ein Kap daneben. Käme je eine dritte dazu, bekäme sie ein lineares Gewicht und die Diagnose sagt es
- **Ein Pflicht-Kap schlägt die Punktzahl.** Unter dem Trefferkap gehen Schläge daneben — das ist keine Abwägung. Ein Kap aus dem Spec-Profil wird deshalb erreicht, wenn es irgend geht, auch wenn jede Umschmiedung dorthin Punkte kostet (der Fall, in dem Krit höher gewichtet ist als Treffer). Ist es ausser Reichweite, wird es nicht zum Vorwand, die ganze Ausrüstung in einen wertlosen Wert zu schmieden. Eine Tempo-Stufe ist dagegen ein Angebot und trägt diese Pflicht nicht
- **`data/reforge_scaling.lua`: exakte Itemwerte je Aufwertungsstufe.** Ein Teil auf 4/4 trägt gut 16 % mehr Wertung; wer das überschlägt, liegt bei jedem Betrag um denselben Prozentsatz daneben. Die Datei trägt dieselben Spieldaten (Budget je Gegenstandsstufe, Statverteilung je Vorlage), aus denen ReforgeLite rechnet. Nur wenn eine Vorlage dort fehlt, bleibt die Budgetkurve 1,15 je 15 Stufen als Näherung — und die Diagnose sagt je Zeile, welcher Weg benutzt wurde. Es sind Spieldaten, keine Handpflege, und damit ausdrücklich **kein** Fall für die Regel aus `data/enchants.lua`
- **Die Statumwandlungen sind vollständig:** Willenskraft als Zaubertreffer, **Waffenkunde als Zaubertreffer**, die Sonderfälle des Nebelwirkers (Willenskraft je zur Hälfte, +50 % Tempo) und die Menschen-Willenskraft. Das sind Spielregeln, keine Feinheiten — ohne die Waffenkunde-Umwandlung wäre Waffenkunde für einen Magier ein toter Wert, den der Planer wegschmiedet, obwohl sie direkt in sein Trefferkap läuft. Dazu die Verstärkungs-Schmuckstücke aus dem Schlund der Donnerkönige, die Tempo, Meisterschaft und Willenskraft prozentual anheben
- **Gerechnet wird in Häppchen.** Ein voller Satz von sechzehn Teilen sind rund 350 ms. Der Lauf ist eine Koroutine, die nach **Zeit** anhält (8 ms, gemessen am Zeitgeber des Clients) und nicht nach einer festen Schrittzahl — die wäre auf einem schnellen Rechner Verschwendung und auf einem langsamen ein Ruckler. Solange gerechnet wird, sagen Seite und Fenster genau das
- **Es gibt weiterhin nur eine Rechnung für „wieviel bringt dieser Wert noch".** Caps und Tempo-Treppe stehen seit 2.6.2.0 in `modules/charakter.lua`; `modules/reforge_engine.lua` rechnet sie **nicht** nach, sondern liest `scan.caps`, `scan.breakpoints` und `scan.profile.statWeights` aus `WeintCodex.Charakter.Scan()`. Genau hier weicht der Planer von ReforgeLite ab, und zwar mit Absicht: dort trägt der Nutzer Gewichte und Kapgrenzen von Hand ein, hier stehen sie schon im Spec-Profil. Zwei Rechnungen nebeneinander wären die Doppelung, an der die Sockelbewertung über fünf Releases gescheitert ist
- **Die Summen kommen vom Charakterbogen, die Einzelbeträge vom Gegenstand.** Ausgangspunkt ist die Kampfwertung des Clients, aus der die angelegten Umschmiedungen herausgerechnet werden. Damit stimmt die Summe immer, und „alles so lassen" ergibt exakt wieder den Istwert
- **Die Paartabelle wird erzeugt, nicht abgetippt** (`data/reforge.lua`). `C_Reforge.ReforgeItem` bekommt keine Statnamen, sondern eine **laufende Nummer** in der Liste der für diesen Gegenstand zulässigen Umschmiedungen. Stünde unsere Tabelle auch nur an einer Stelle anders herum, schmiedete der Knopf etwas anderes als angezeigt, und das fiele erst auf, nachdem Gold ausgegeben wurde. `WeintCodex_ValidateReforgeData()` prüft beim Login zusätzlich, dass die Statnummern der Skalierungsdaten dieselben sind — wären sie verschoben, bekäme jeder aufgewertete Gegenstand lautlos die Werte eines anderen Wertes zugeschrieben
- **Ist ein Paar für den Gegenstand nicht zulässig, kommt keine Nummer zurück** und die Zeile wird übersprungen. Die Zahl an der Stelle trotzdem herauszugeben wäre die Nummer einer *anderen* Umschmiedung
- **Der Umschmiedewert im Item-Link** wird nicht an einer geglaubten Feldposition gelesen, sondern am Wertebereich erkannt (113…168) und an der Zeichenkette `REFORGED` des Clients gegengeprüft. **Diese Gegenprobe widerspricht, sie verbietet nicht:** ein Widerspruch steht an der Zeile und in der Diagnose, die Zeile bleibt planbar. Was der Umschmieder anlegt, hängt an der laufenden Nummer und die allein an den Werten des Grundgegenstands. Und stimmte unsere Lesart einmal nicht, wäre das Werkzeug sonst auf einen Schlag für jeden umgeschmiedeten Gegenstand tot; eine Prüfung, die im Zweifel alles abschaltet, ist keine
- **Während eines Laufs wird nicht neu geplant.** Nach jedem umgeschmiedeten Teil ändert sich der Item-Link und damit die Ausgangslage; ein Planer, der mitten im Lauf seine Meinung ändert, schmiedet das nächste Teil anders als in der Liste steht, auf die geklickt wurde. Die Haken kommen trotzdem aus dem Item-Link und sind damit der echte Stand
- **Die Kennung für „muss neu gerechnet werden" wird billig gebildet** — nur aus Item-Links, Sperren, Spec und den acht Kampfwertungen, ohne Tooltip und ohne Ausrüstungs-Scan. Das Neuzeichnen ist ausserdem entprellt (1,5 s), weil `PLAYER_EQUIPMENT_CHANGED` beim Umsockeln mehrfach feuert
- `REFORGE_SHARE` in `modules/charakter.lua` liest jetzt `WeintCodex_Reforge.COEFF`; zwei Konstanten für dieselben 40 % wären die Doppelung, aus der Empfehlung und Urteil auseinanderlaufen
- Der Eintrag hängt in der Charakter-Seitenleiste und nicht in der Navigationsspalte: dort ist kein Platz mehr (676 von 684 px), und die Frage „wohin mit der Wertung" gehört ohnehin neben *Sockel* und *Werteverteilung*. Gezeichnet wird die Seite von `modules/reforge.lua`, deshalb ruft sie `WeintCodex.Charakter.LeaveView()` auf
- **Gegen einen nachgebauten Client geprüft, 31 Prüfungen** — darunter die Verzahnung selbst, an einem Krieger weit unter dem Trefferkap: ohne Planer empfiehlt die Sockelseite einen Trefferstein, mit Planer keinen mehr, jede Zeile nennt den Grund, und ein Stein über dem Cap gilt nicht mehr als verschwendet, sobald der Plan den Überschuss wegräumt. Dazu: Darunter: der Planer gegen eine **rohe Gewalt über alle 371 293 Kombinationen** einer Fünf-Slot-Aufstellung (gleiches Ergebnis); ein Pflicht-Kap, das Punkte kostet und trotzdem erreicht wird; ein unerreichbares Kap, das die Rechnung nicht kippt; Waffenkunde, die auf einem Magier liegen bleibt; die Aufwertung 4/4 exakt aus der Skalierungstabelle; Paartabelle und laufende Nummer gegen eine von Hand nachgezählte Liste; der Widerspruch zwischen Client und Link; gesperrte Slots; ein fehlendes Spec-Profil; der Seitenaufbau in beiden Schaltzuständen und ein vollständiger Umschmiede-Lauf über mehrere Runden
...
### Neu
- **Tempo-Schwellen in der Sockel- und Verzauberungsprüfung.** Ein periodischer Effekt gewinnt einen Tick, sobald genug Tempo da ist; dazwischen ändert sich an der Tickzahl nichts. Genau dieser Treppe folgt der Planer jetzt: unterhalb der angepeilten Stufe zählt Tempo, darüber nicht mehr — dieselbe Mechanik (`headroom`), mit der das Trefferkap seit 2.5.0.0 rechnet. Ein Tempostein hinter der Schwelle wird deshalb nicht mehr empfohlen, und ein angelegter fällt als *hinter der Schwelle* auf
- **Die Stufen werden gerechnet, nicht abgeschrieben.** `data/breakpoints.lua` enthält je Effekt nur **Laufzeit und Tickabstand** — zwei Zahlen, die jeder im Zauberbuch nachsehen kann. Die Prozentsätze folgen daraus (`Ticks = round(Laufzeit × (1 + Tempo) / Grundabstand)`, also Stufe N bei `(N − 0,5) × Abstand / Laufzeit − 1`). Eine Tabelle abgeschriebener Wertungszahlen („3.043 Tempo") wäre das Gegenteil gewesen: sie gilt für eine Ausrüstungsstufe, eine Buffkombination und einen Guide, und niemand könnte ihr ansehen, ob sie noch stimmt
- **Umschmieden zählt mit.** Ob die nächste Stufe überhaupt zu erreichen ist, entscheidet die *Reichweite*: 40 % eines Sekundärwerts je Gegenstand plus die freien Sockel. Liegt eine Stufe darin, ist die höchste davon das Ziel und Tempo zählt bis dorthin; liegt keine mehr drin, ist die zuletzt erreichte das Ende und der Rest gehört umgeschmiedet. Die Reichweite ist bewusst eine **Obergrenze** — eine zu große lässt die Stufe als erreichbar gelten und ändert dann gar nichts, eine zu kleine würde Tempo kappen, das noch etwas bringt
- **Jeder Cap-Befund nennt jetzt das Umschmieden.** Bis hierher endete er mit „Umsockeln!" — das ist die halbe Antwort und meistens die teurere: Wertung über einer Grenze steht nicht falsch *im* Stein, sie steht im falschen Stat. Die Texte nennen jetzt die Menge, die daneben liegt, **und** wohin damit (der höchstgewichtete Sekundärwert, der nicht selbst an einer Grenze steht)
- **Die Verzauberungsempfehlung folgt derselben Regel.** Läuft die erste Verzauberung einer Slot-Liste komplett ins Leere — 170 Tempo hinter der Schwelle, 175 Treffer über dem Kap —, wird die nächste vorgeschlagen, die noch etwas bringt. Eine Verzauberung ist Wertung wie ein Stein
- **`/wc tempo`** (auch als Knopf unter *Einstellungen → Diagnose*): die Treppe der eigenen Spec mit ihrer Herleitung je Stufe, welche Client-Funktion den Istwert gemeldet hat, der Buff-Faktor, die Wertung je Prozentpunkt, die Umschmiede-Reserve und das gerechnete Ziel. Eigener Befehl aus demselben Grund wie `/wc sockel`: „es wird ein Tempostein vorgeschlagen, obwohl ich am Cap bin" sieht bei einer falschen Laufzeit in der Datendatei, einem stummen Client, einer zu groß geschätzten Reichweite und einem selbst gesetzten Ziel von außen identisch aus

### Geändert
- **Die Seite *Werteverteilung & Caps* zeigt die Treppe** — Istwert, Ziel, erreichte und kommende Stufen mit dem Abstand in Wertung, und woraus sich die Reichweite zusammensetzt. Jede Stufe trägt einen Knopf *als Ziel*, dazu *Automatisch* und *Schwellen aus*: **welche Stufe man anpeilt, ist eine Entscheidung, keine Messung.** Wer eine Stufe über einen Schmuckproc erreicht oder einem Guide folgt, der etwas anderes sagt, weiß mehr als diese Rechnung — und die Seite sagt dann auch „selbst gesetzt" statt „nicht erreichbar"
- **Die Seite hat einen Bildlauf.** Mit dem dritten Abschnitt lief sie unten heraus, statt abgeschnitten zu werden — unerreichbar, dieselbe Falle wie beim Changelog-Popup vor 2.4.0.0
- Der Inspektor der Seite heißt *Caps & Schwellen* und führt beide

### Technisch
- **Ein Eintrag in `data/breakpoints.lua` ist selbst eine Aussage:** für diese Spec ist Tempo ein Schwellenwert, und hinter der letzten erreichbaren Stufe gehört es umgeschmiedet. Für eine Spec, deren Tempo vor allem Zauberzeit und Ressourcen bringt, wäre dieselbe Aussage falsch — solche Specs stehen deshalb **nicht** darin, und für sie ändert sich nichts. Eine fehlende Spec ist kein Versäumnis, sondern die ehrlichere Auskunft (dieselbe Regel wie bei den Rotationslisten der Tankspecs). Enthalten sind die sieben DoT-/HoT-Specs, bei denen „bis zur Stufe und dann umschmieden" der Rat jedes Guides ist
- **Die Laufzeiten tragen `verify = true`** — sie stammen aus der Datenlage zu MoP 5.4 und sind am Client dieser Installation nicht geprüft. Benutzt werden sie trotzdem (wie in `data/enchants.lua`), und `/wc tempo` druckt zu jeder Stufe ihre Herleitung aus, damit ein falscher Wert auffällt statt stillschweigend Empfehlungen zu tragen
- **`WeintCodex_ValidateBreakpointData()`** läuft beim Login mit den anderen Datenprüfungen: sie meldet, wenn Laufzeit und Tickabstand keine ganze Grundtickzahl ergeben — dann stimmt eine der beiden Zahlen nicht, und aus ihr fällt eine ganze Treppe. Ob 18 Sekunden wirklich 18 Sekunden sind, kann sie nicht wissen; dafür ist `verify` da
- **Der Buff-Faktor wird herausgerechnet.** Der Charakterbogen meldet den Gesamtwert (inklusive Raidbuffs), die Kampfwertung nur ihren eigenen Anteil; Buffs wirken multiplikativ. Wer den Abstand zur nächsten Stufe — einen Gesamtwert — mit der reinen Wertungsumrechnung multipliziert, verlangt deshalb zu viel Wertung. Der Faktor fällt aus den beiden Zahlen heraus, die der Client ohnehin meldet
- **Decken und Treppe speisen denselben Spielraum** (`headroom` in `ScanCharacter`). Für den Planer ist es dieselbe Frage — wieviel Wertung bringt in diesem Stat überhaupt noch etwas —, und zwei Rechnungen nebeneinander wären genau die Doppelung, aus der Empfehlung und Urteil auseinanderlaufen; das ist die Lehre aus `PlanItem`. Auch `/wc sockel` liest jetzt beide, sonst rechnete die Diagnose mit anderen Zahlen als die Seite
- **`headroom == nil` heißt „keine Aussage".** Meldet der Client den Istwert nicht, ist noch keine Stufe erreicht und keine in Reichweite, oder hat der Spieler die Schwellen abgeschaltet, bleibt Tempo ungedeckelt statt mit 0 als wertlos zu gelten. Eine Aussage über unsere Rechenlage ist keine Aussage über die Ausrüstung
- **Umgereiht wird eine Verzauberungsliste nur, wenn die erste komplett ins Leere läuft.** „Etwas weniger wert" ist kein Grund, eine kuratierte Liste umzusortieren — dieselbe Regel wie beim Kandidatentopf der Steine. Proc-Verzauberungen (Tanzender Stahl, Jadegeist, DK-Runen) haben bewusst keine Werte und werden nie verdrängt; und das **Urteil** über eine angelegte Verzauberung ändert sich dadurch nicht: was in der Liste steht, bleibt optimal
- „Über Cap" und „hinter der Schwelle" stehen als getrennte Texte an der Zeile (`capKind`): über dem Trefferkap ist die Wertung wertlos, hinter der letzten erreichbaren Tempo-Stufe bringt sie nur keinen Tick mehr. Ein Text für beides wäre für einen der beiden Fälle falsch
- `SM.NormalizeItemStats` / `SM.ItemStats` in `modules/stat_match.lua` exportieren die `ITEM_MOD_*`-Zuordnung des Clients — die Umschmiede-Reserve braucht die Werte angelegter Gegenstände, und eine zweite Abschrift dieser Zuordnung wäre die Doppelpflege, an der die Verzauberungserkennung schon einmal gescheitert ist
- Das Ziel je Spec liegt unter `SavedData.statTargets[<Profilschlüssel>]`, neben `customWeights` und aus demselben Grund **nicht** in `GUILD_KEYS`: es ist eigene Einstellung, nichts Gildeninternes
- Gegen einen nachgebauten Client geprüft: bei 12 %, 26,4 %, 33 % und 45 % Tempo, mit gerechnetem Ziel, selbst gesetztem Ziel und abgeschalteten Schwellen. Bei erreichtem Ziel wechselt die Empfehlung vom *Spiegelnden Goldberyll* (Tempo) zum *Frakturierten Goldberyll* (Meisterschaft), ein angelegter Tempostein fällt als *hinter der Schwelle* auf, und ohne Schwellen steht wieder überall Tempo

## [2.6.1.1] – 2026-08-26

Nachtrag zu 2.6.1.0: der Kalender-Eintrag lud danach immer noch nur **einen** Spieler ein.

### Behoben
- **Nach dem Realm-Fix kam trotzdem nur eine einzige Einladung durch** („2 von 25 bestätigt" — der Ersteller und der erste Name). Zwei Dinge daran waren falsch:
  - **Gewartet wurde nach einer festen Frist statt auf Fortschritt.** 24 Versuche à 0,5 s, begründet damit, zwölf Sekunden seien „grosszügig für eine Namensauflösung" — das ist eine Aussage über *einen* Namen. Fünfundzwanzig davon beantwortet der Server nicht in zwölf Sekunden, und danach stand als Ergebnis da, die Charaktere gebe es nicht. Eine feste Frist macht aus „der Server ist noch nicht fertig" eine Tatsachenbehauptung über fremde Charakternamen. Gewartet wird jetzt, solange die Zahl der Bestätigungen **steigt**; erst wenn sie steht, ist die Stille eine Antwort
  - **Alle Einladungen gingen in einem einzigen Frame raus.** Das muss der erste Durchlauf, denn er hängt am Klick — aber der Server beantwortet die Namensauflösung nicht im Bulk: von 25 Anfragen kommt genau die erste an. Belegt mit `/wc kalender` am gemeldeten Fall, und zwar so, dass es an den Namen *nicht* liegen kann — `Cynsaria` steht auf dem eigenen Realm und wurde als nackter Name angefragt, `Féritas-Everlook` wurde sogar realmfremd aufgelöst; angekommen ist trotzdem nur die erste Anfrage. Steht der Stand, wird deshalb **einzeln nachgefasst, und immer nur eine Anfrage gleichzeitig**: die nächste geht raus, sobald die vorige bestätigt ist. Der Lauf taktet sich damit am Server statt an einer geratenen Zahl — ein fester Abstand wäre entweder zu kurz (dann fällt wieder die Hälfte weg) oder zu lang. Ein Name, den es nicht gibt, wird nie bestätigt und hält den Rest nach `RESEND_GAP` trotzdem nicht auf
  - **Die zweite Runde nimmt die andere Schreibweise** (`Name-Realm` ↔ nackter Name). Denselben String ein zweites Mal zu schicken, nachdem der Server ihn schon abgelehnt hat, brächte keine neue Auskunft; die andere Schreibweise schon — und der Realm ist genau der Teil der Zeile, der fehlen oder falsch sein kann, weil er von Hand gepflegt wird

### Technisch
- Die zweite Schreibweise wird **erst nach einer Ablehnung** probiert und nie blind: gäbe es den Namen auf beiden Realms, lüde eine Blindprobe den Falschen mit ein. Nach einer Ablehnung ist dieses Risiko klein und die Auskunft die einzige, die noch zu holen ist
- Runden aussen, Namen innen. Andersherum — beide Runden eines Namens direkt hintereinander — bekäme der Letzte der Liste seine erste Nachfrage erst, wenn alle vor ihm zweimal durch sind; genau diese Reihenfolge hilft niemandem, wenn der Server drosselt, und gedrosselt zu werden ist der Verdacht, aus dem heraus überhaupt nachgefasst wird
- `WATCH_STALL` von 16 auf 8 Takte (4 s). Länger auf die übrigen 24 aus dem ersten Schwung zu warten kostet nur Zeit, und jeder Zuwachs setzt den Zähler ohnehin zurück
- Gegen einen simulierten Server geprüft: der gemeldete Fall ist nach 16 s vollständig, ein Server mit drei Takten Antwortzeit nach 39 s, ein stummer Server gibt nach 52 s auf, und drei wirklich unbekannte Namen halten die übrigen zwanzig nicht auf
...
### Behoben
- **Der Kalender-Eintrag lud niemanden ein: „1 von 25 bestätigt", 24 Namen „nicht gefunden".** Zwei Ursachen, und beide zeigten dasselbe Bild:
  - **Der eigene Realm wurde in der falschen Schreibweise verglichen.** Gespeichert wird ein Realm überall ohne Leerzeichen (`modules/companion.lua` beim Melden der Twinks, `core/names.lua` beim Zerlegen — „Ook Ook" wird zu „OokOok"), verglichen wurde er in `modules/calendar.lua` aber gegen das rohe `GetRealmName()`, also gegen die Anzeigeform *mit* Leerzeichen. Auf jedem Realm, dessen Name ein Leerzeichen enthält, ging der Vergleich damit **nie** auf: auch der Nachbar vom eigenen Realm bekam ein „-OokOok" angehängt, und `Njiah-OokOok` kennt der Server als Charakter des eigenen Realms nicht. Verglichen wird jetzt über `WeintCodex.Names.Me()` — dieselbe Normalisierung wie überall sonst
  - **Ein fehlgeschlagener Einladungsaufruf war nicht von einem unbekannten Namen zu unterscheiden.** `pcall(C_Calendar.EventInvite, name)` warf seinen Rückgabewert weg, mit einem Kommentar daneben, der das begründete: er sage nur, dass kein Lua-Fehler auftrat. Das stimmt — und verschluckt genau den Fall, in dem es die Funktion gar nicht gibt. `pcall` auf einen `nil`-Wert liefert `false, "attempt to call a nil value"` und wirft nichts; 25 stille Fehlschläge sehen danach exakt aus wie 25 unbekannte Charaktere, der Eintrag entsteht, der Ersteller steht allein drin, und die Meldung beschuldigt die Schreibweise. **Dieselbe Lehre wie beim Signalton des Ausrüstungs-Alarms:** ein Rückfallweg, dessen Zweige alle schweigend scheitern können, ist kein Rückfallweg
- **Die Fehlermeldung riet zu der Schreibweise, die der Fehler war.** „Crossrealm: Name-Realm" — genau das Format, das das Addon ohnehin anhängt. Sie nennt jetzt den eigenen Realm und die Regel dazu, und unterscheidet „der Server kennt den Namen nicht" von „der Aufruf ist nie rausgegangen": das sind zwei Antworten, und sie raten zu Verschiedenem
...
### Technisch
- **`/wc kalender`** druckt, was der Client an Kalenderfunktionen hergibt, welchen Realm das Addon vergleicht (roh und normalisiert) und welcher Name je Spieler angefragt würde. Eigener Befehl aus demselben Grund wie `/wc sockel` und `/wc vz zeilen`: „24 von 25 nicht gefunden" sieht bei einer fehlenden Client-Funktion, einem falsch verglichenen Realm und einem wirklich falschen Charakternamen identisch aus — von aussen war diese Fehlerklasse nicht diagnostizierbar, und das ist der Grund, warum sie so lange stand
- Die Einladungsfunktion wird **einmal aufgelöst und benannt** (`INVITE_FUNCS`/`ResolveInviteFunc`), mehrere Kandidaten, weil sich der Name je nach Client-Stand unterscheidet und keiner davon von hier aus zu belegen ist. Gibt es keinen, ist das eine Aussage und keine Vermutung — gemeldet wird sie in der Statuszeile, nicht als stilles Nichts
- Der Befund reist am Entwurf mit (`draft.blocked`) statt sofort gemeldet zu werden: die Statuszeile gehört ab der Vorbereitung `WatchDraft`, und was `PrepareIngameCalendarEvent` hineinschriebe, wäre eine Zehntelsekunde später überschrieben
...
- **Zwei Verzauberungs-IDs standen falsch in der Datenbank** — gemeldet am Heiligpriester, dessen Ausrüstung nachweislich richtig verzaubert war und trotzdem zweimal „(ID abweichend – /wc vz)" zu lesen bekam. Beide Male stimmte der Client, nicht die Tabelle:
  - **Handschuhe, ID 4433:** hier stand *Großes Tempo* (Tempo 170), der Item-Tooltip zeigt **+170 Meisterschaft**. Der Eintrag war aus der WoWHead-Itemnummer 74719 abgeleitet und nie am Client geprüft, während für 4430 = *Überragende Meisterschaft* ein Tooltip vorlag. 4433 ist damit die **zweite** Meisterschafts-ID neben 4430 — zwei IDs für dieselbe Verzauberung sind in dieser Datei die Regel (4422/4424 Umhang, 4432/4434 Hände)
  - **Beine, ID 4826:** hier stand der *himmelblaue* Zauberfaden (Intelligenz + Krit), der Tooltip zeigt **+285 Intelligenz und +165 Willenskraft** — also den *perlmuttfarbenen*. Die Namen selbst waren richtig (Pearlescent = Willenskraft, Cerulean = Krit); vertauscht war, welche ID welchen Faden meint. Bestätigt worden war seinerzeit nur die Schreibweise des Namens, nicht die Zuordnung
- **Der Heiler bekam dadurch den richtigen Zauberfaden als zweite Wahl vorgehalten.** Die Empfehlungslisten aller 13 betroffenen Specs sind mitgezogen: Heiler (Heilig-Paladin, Disziplin, Heilig, Wiederherstellung Schamane/Druide, Nebelwirker) führen jetzt den perlmuttfarbenen Faden zuerst, Zauber-DPS (Magier, Hexenmeister, Gleichgewicht) den himmelblauen. **Schatten und Elementar standen dabei schon vorher falsch** — sie führten den Willenskraft-Faden zuerst und stehen mit der neuen Zuordnung ohne Änderung richtig

### Technisch
- **Die Tempo-Handschuhe stehen jetzt unter ihrer Itemnummer** (`[74719]`, `verify`), weil ihre Verzauberungs-ID am Client unbelegt ist — dieselbe Notlösung wie bei 74715 (Stiefel-Tempo) und 74711 (Umhang-Ausdauer). Ohne sie hätten die 13 Tempo-Specs keine Handschuh-Empfehlung mehr, und wer richtig auf Tempo verzaubert hat, bekäme seine Werte gegen eine Meisterschaftsempfehlung gehalten — also einen Mangel gemeldet, den es nicht gibt. Erkannt wird der Eintrag über Slot + Werte bzw. den Namen, nie über die Nummer
- Wer die Handschuhe mit der 4433 trägt, liest weiterhin „Optimal": die ID steht bewusst in keiner Empfehlungsliste (wie 4434), der Werteabgleich erkennt sie als *werte-identisch* — genau der Fall, für den er da ist
- `WeintCodex_ValidateSpecData()` meldet nach der Änderung 0 ungültige ID-Referenzen; die erste Empfehlung jeder Spec für Hände und Beine ist gegen die Werte gegengeprüft
