...
Das Werkzeug ist eine **Beta** und deshalb **ab Werk aus**. Es gibt Gold aus, und was es ausrechnet, ist noch nicht geprüft.

### Neu
- **Die Seite *Charakter → Umschmieden*.** Oben, was die Grenzen deiner Spec sagen — Istwert, Stand nach dem Plan und das Ziel auf einer Bahn. Darunter je Teil, was von wo nach wo geht, wieviel das ist, und **warum**: *füllt Trefferwertung*, *über Waffenkunde*, oder welches Gewicht das andere schlägt. Die Begründung wird nicht dazugeschrieben, sie steht an der Entscheidung, die getroffen wurde
...
- **`/wc umschmieden`** öffnet die Seite, `an`/`aus` schaltet, `fenster` zeigt die Liste auch ohne Umschmieder, und **`/wc umschmieden prüfen`** druckt jede Zwischenzahl: gelesene Itemwerte, Aufwertungsfaktor, angelegte Umschmiedung, das gerechnete Ziel je Wert und die laufende Nummer, die der Umschmieder bekäme

### Geändert
- Die Unterseite steht nur da, wenn der Planer eingeschaltet ist. Ein ausgegrauter Reiter verspricht etwas, das nicht da ist

### Technisch
- **Es gibt weiterhin nur eine Rechnung für „wieviel bringt dieser Wert noch".** Caps und Tempo-Treppe stehen seit 2.6.2.0 in `modules/charakter.lua` und speisen dort den Spielraum der Sockelplanung; `modules/reforge_engine.lua` rechnet sie **nicht** nach, sondern liest `scan.caps`, `scan.breakpoints` und `scan.profile.statWeights` aus `WeintCodex.Charakter.Scan()`. Zwei Rechnungen nebeneinander wären genau die Doppelung, an der die Sockelbewertung über fünf Releases gescheitert ist — die Umschmiedeseite behauptete sonst ein anderes Trefferkap als die Seite daneben
...
- **Itemwerte kommen vom Grundgegenstand** (`GetItemStats("item:<id>")`), nicht vom Link des angelegten Teils: umgeschmiedet wird, was der Gegenstand selbst trägt, nicht der Stein darin oder die Verzauberung darauf. Die Aufwertungsstufe fehlt dem Grundgegenstand und wird mit der Budgetkurve von MoP (1,15 je 15 Gegenstandsstufen) über den Abstand hochgerechnet, den der **Client** zwischen Link und blankem Gegenstand meldet — keine Aufwertungstabelle zum Pflegen. Das ist eine Näherung, sie steht als solche in `/wc umschmieden`, und sie wirkt nur auf die Einzelbeträge, nie auf die Summen
- **Die Paartabelle wird erzeugt, nicht abgetippt** (`data/reforge.lua`). `C_Reforge.ReforgeItem` bekommt keine Statnamen, sondern eine **laufende Nummer** in der Liste der für diesen Gegenstand zulässigen Umschmiedungen — und die entsteht, indem der Client seine eigene Paartabelle von oben nach unten durchgeht. Stünde unsere auch nur an einer Stelle anders herum, schmiedete der Knopf etwas anderes als angezeigt, und das fiele erst auf, nachdem Gold ausgegeben wurde. Die Regel („für jede Quelle jedes Ziel außer sich selbst", 8 × 7 = 56, Umschmiedewert 112 + Nummer) kann keinen Zahlendreher enthalten; `WeintCodex_ValidateReforgeData()` prüft beim Login, was daran überhaupt zu prüfen ist
- **Ist ein Paar für den Gegenstand nicht zulässig, kommt keine Nummer zurück** und die Zeile wird übersprungen. Die Zahl an der Stelle trotzdem herauszugeben wäre die Nummer einer *anderen* Umschmiedung — der eine Fehler dieser Dateien, der Gold kostet
- **Der Umschmiedewert steht in einem Feld des Item-Links, dessen Position sich zwischen Clientständen verschoben hat.** Statt eine davon zu glauben, wird der Bereich abgesucht, in dem sie liegen kann, und dort das Feld genommen, dessen Wert ein gültiger Umschmiedewert **ist** (113…168). Kein anderes Feld dieses Bereichs kann in diesem Zahlenraum liegen. Gegengeprüft wird an der eigenen Zeichenkette des Clients (`REFORGED`): das ist die Ja/Nein-Auskunft, ob überhaupt etwas umgeschmiedet ist
- **Diese Gegenprobe widerspricht, sie verbietet nicht.** Ein Widerspruch steht an der Zeile und in der Diagnose, die Zeile bleibt planbar. Was der Umschmieder anlegt, hängt an der laufenden Nummer und die allein an den Werten des Grundgegenstands — eine falsch gelesene *angelegte* Umschmiedung macht die Beträge ungenau, nicht den Auftrag falsch. Und stimmte unsere Lesart von `REFORGED` einmal nicht, wäre das Werkzeug sonst auf einen Schlag für jeden umgeschmiedeten Gegenstand tot; eine Prüfung, die im Zweifel alles abschaltet, ist keine
- **Der Suchlauf braucht keine erfundene Cap-Prämie.** Bewertet wird Gewicht mal Wertung, aber nur bis zum Ziel — dieselbe Regel wie `GemValue`. Daraus fällt das gewünschte Verhalten von selbst: unter dem Kap ist Treffer der höchstgewichtete Sekundärwert und wird gefüllt, darüber ist er null wert und wird weggeschmiedet. Gesucht wird in zwei Runden (einzeln bis zur Ruhe, dann paarweise) von zwei Startaufstellungen aus — die paarweise Runde fängt den Fall, in dem sich erst zwei Änderungen zusammen lohnen, und der ist an einem Kap der Normalfall
- **Willenskraft senkt bei den drei Casterspecs mit `spiritZaehlt` das Trefferziel**, statt in beiden Töpfen gezählt zu werden. Das Ziel steht in reiner Trefferwertung und rechnet mit der Willenskraft, die gerade angelegt ist; verschiebt der Plan sie, muss es mitwandern
- **Während eines Laufs wird nicht neu geplant.** Nach jedem umgeschmiedeten Teil ändert sich der Item-Link und damit die Ausgangslage; ein Planer, der mitten im Lauf seine Meinung ändert, schmiedet das nächste Teil anders als in der Liste steht, auf die geklickt wurde. Die Haken an der Liste kommen trotzdem aus dem Item-Link und sind damit der echte Stand
- **Die Kennung für „muss neu gerechnet werden" wird billig gebildet** — nur aus Item-Links, Sperren, Spec und den acht Kampfwertungen, ohne Tooltip und ohne Ausrüstungs-Scan. Das Fenster fragt während eines Laufs nach jedem Gegenstand nach
...
- Der Eintrag hängt in der Charakter-Seitenleiste und nicht in der Navigationsspalte: dort ist kein Platz mehr (676 von 684 px), und die Frage „wohin mit der Wertung" gehört ohnehin neben *Sockel* und *Werteverteilung*. Gezeichnet wird die Seite von `modules/reforge.lua`, deshalb ruft sie `WeintCodex.Charakter.LeaveView()` auf — sonst legte der Ausrüstungs-Watcher die zuletzt gezeigte Charakterseite darüber
- Gegen einen nachgebauten Client geprüft: Paartabelle und laufende Nummer gegen eine von Hand nachgezählte Liste, das Lesen des Umschmiedewerts aus dem Link, die Aufwertungsrechnung, der Widerspruch zwischen Client und Link, das Auffüllen und das Abbauen eines Kaps, gesperrte Slots, ein fehlendes Spec-Profil, der Seitenaufbau in beiden Schaltzuständen und ein vollständiger Umschmiede-Lauf über mehrere Runden
